...
const getSpecificMovie = async (req, res) => {
  try {
    ...
    if (!specificMovie) return res.status(404).json({ message: "Movie not found" });
...
// NEW: Get movies by genre (for "similar movies")
const getMoviesByGenre = async (req, res) => {
  try {
    const { genreId } = req.params;
    const movies = await Movie.find({ genre: genreId }).limit(6);
    res.json(movies);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
